...
def grep_file(filename, pattern, find, line_number, color, no_filename):
    text = sys.stdin if filename == '-' else open(filename, 'r')
    line = text.readline()
...
def grep_files(paths, pattern, find, recurse, lineno, color, no_filename):
    for path in paths:
        if os.path.isfile(path):
            grep_file(path, pattern, find, lineno, color, no_filename)
        else:
            if recurse:
                more_paths = [path + '/' + child for child in os.listdir(path)]
                grep_files(more_paths, pattern, find, recurse, lineno, color, no_filename)
            else:
...
def main():
    ...
    grep_files(args.files, pattern, find, args.recursive, args.line_number, args.color, args.no_filename)
